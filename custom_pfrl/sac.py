...
class SoftActorCritic(AttributeSavingMixin, BatchAgent):
    ...
    def batch_act(self, batch_obs, skip_mask=None):
        if self.training:
            return self._batch_act_train(batch_obs, skip_mask)
        else:
            return self._batch_act_eval(batch_obs)

    def batch_observe(self, batch_obs, batch_reward, batch_done, batch_reset, skip_mask=None):
        if self.training:
            self._batch_observe_train(batch_obs, batch_reward, batch_done, batch_reset, skip_mask)
    ...
    def _batch_act_train(self, batch_obs, skip_mask=None):
        if skip_mask is None:
            skip_mask = np.zeros((len(batch_obs),))
        try:
            old_batch_last_obs = self.batch_last_obs.copy()
            old_batch_last_action = self.batch_last_action.copy()
        except:
            pass
        ...
        for i, truth in enumerate(skip_mask):
            if truth:
                self.batch_last_obs[i] = old_batch_last_obs[i]
                self.batch_last_action[i] = old_batch_last_action[i]

        return batch_action

    def _batch_observe_train(self, batch_obs, batch_reward, batch_done, batch_reset, skip_mask=None):
        assert self.training
